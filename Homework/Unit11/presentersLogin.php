...
$message = "";
	...
	if (isset($_SESSION['validUser']) && $_SESSION['validUser'] == "yes")				//is this already a valid user?
	{
		...
		$message = "Welcome Back" . $_SESSION['userName'];	//Create greeting for VIEW area
	}
	else
	{
		if (isset($_POST['submitLogin']) )			//Was this page called from a submitted form?
		{
			...
			$sql = "SELECT username, password FROM wdv341_presenter WHERE username = ? AND password = ?";

			$query = $conn->prepare($sql) or die("<p>SQL String: $sql</p>");	//prepare the query

			$query->bindParam(1, $inUsername , PDO::PARAM_STR, 20);	//bind parameters to prepared statement
			$query->bindParam(2, $inPassword , PDO::PARAM_STR, 20);
